  ...
  useEffect(() => {
    const loadDashboardData = async () => {
        ...
        setStats({
          ...
          pendingApproval: allFeedbacks.filter(f => f.approvalStatus === ApprovalStatus.PENDING).length,
    ...
    <div className="p-8 h-full overflow-y-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-12">
        ...
        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
           <div className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-indigo-200 dark:shadow-none">
             {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
           </div>
           <span className="text-xs font-black text-indigo-600 dark:text-indigo-400 px-3 uppercase tracking-wider">
           ...
           <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 h-full">
              ...
              <div className="p-4 border-t border-slate-100 dark:border-slate-800">
                <button 
                  ...
                  className="w-full py-3 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 dark:shadow-none transition-all active:scale-95"
                >
                  Create New Report
                </button>
              </div>
