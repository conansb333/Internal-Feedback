        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex bg-white dark:bg-slate-800 rounded-lg p-1 border border-slate-200 dark:border-slate-700 shadow-sm">
                <button 
                  ...
                  className={`p-2 rounded-md transition-colors ${viewMode === 'list' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                  title="List View"
                >
                ...
                <button 
                  ...
                  className={`p-2 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                ...
                <button 
                  ...
                  className={`p-2 rounded-md transition-colors ${viewMode === 'details' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                  title="Details View"
                >
            ...
            {isManager && !isCreating && (
              <Button onClick={() => setIsCreating(true)} className="shadow-lg shadow-indigo-200 dark:shadow-none whitespace-nowrap">
        ...
        <div className="mb-8 bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl shadow-xl border border-indigo-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-4 relative overflow-hidden">
          ...
          <form onSubmit={handleCreate}>
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-6">
              ...
              <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Options</label>
            ...
            <div className="mb-6">
               ...
               <input 
                 ...
                 className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-lg"
                  ...
                  <div className="flex bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
                      {(['sm', 'base', 'lg'] as const).map(size => (
                        <button
                          ...
                          className={`px-2 py-1 text-xs font-bold border-r last:border-r-0 border-slate-200 dark:border-slate-700 ${textSize === size ? 'bg-indigo-50 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                        >
           ...
           announcements.map(item => {
             ...
             if (viewMode === 'grid') {
                 return (
                    <div 
                        key={item.id}
                        className="group bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-5 hover:shadow-lg transition-all flex flex-col relative"
                    >
                        {item.isImportant && <div className="absolute top-3 right-3 w-2 h-2 rounded-full bg-red-500 animate-pulse"></div>}

                        <div className={`w-10 h-10 rounded-xl mb-4 flex items-center justify-center ${config.bg} ${config.color}`}>
                            <TypeIcon className="w-5 h-5" />
                        </div>
                        
                        <h3 className="font-bold text-slate-900 dark:text-white mb-2 line-clamp-2 h-12 leading-tight">
                            {item.title}
                        </h3>
                        
                        <p className={`text-sm text-slate-500 dark:text-slate-400 line-clamp-3 mb-4 flex-1 ${item.textColor}`}>
                            {item.content}
                        </p>
             ...
             // --- DETAILS VIEW (Original - No Image) ---
             return (
               <div 
                 key={item.id} 
                 className={`group relative bg-white dark:bg-slate-900 rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col`}
               >
                  <div className="p-6 md:p-8 relative">
                      ...
                      <div className="prose prose-sm prose-slate dark:prose-invert max-w-none mb-6">
                        <p 
                          className={`whitespace-pre-wrap leading-relaxed ${item.textColor || 'text-slate-600 dark:text-slate-300'} ${item.textSize === 'lg' ? 'text-lg' : item.textSize === 'sm' ? 'text-sm' : 'text-base'}`}
                        >
                          {item.content}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-5 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400">
                         <div className="flex items-center gap-2">
                             <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-700 dark:text-slate-300 border-2 border-white dark:border-slate-600 shadow-sm">
                               {item.authorName.charAt(0)}
                             </div>
                             <div>
