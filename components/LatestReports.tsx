...
export const LatestReports: React.FC<LatestReportsProps> = ({ currentUser }) => {
  ...
  return (
    <div className="p-8 h-full overflow-y-auto">
      <div className="mb-12 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
        <div className="flex items-center gap-4">
           <div className="p-3 bg-indigo-600 rounded-2xl shadow-xl shadow-indigo-100 dark:shadow-indigo-900/20">
               <Clock className="w-8 h-8 text-white" />
           </div>
           <div>
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Latest Activity</h2>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
           <div className="relative">
                ...
                <input 
                    ...
                    className="pl-9 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm w-full sm:w-64 focus:ring-2 focus:ring-indigo-500 outline-none"
                />
           </div>
                          ...
                          <div className="flex gap-2">
                              ...
                              <button 
                                ...
                                className={`px-3 py-1.5 text-xs font-bold rounded-lg flex items-center gap-1 transition-colors ${
                                    ...
                                    ? 'bg-slate-100 text-slate-400 cursor-not-allowed dark:bg-slate-800 dark:text-slate-600'
                                    : 'bg-green-600 text-white hover:bg-green-700 shadow-sm shadow-green-100'
                                }`}
                              >
                    ...
                    <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center sticky bottom-0 z-10">
                        ...
                        <div className="flex gap-3">
                            {selectedReport.resolutionStatus === ResolutionStatus.CLOSED_RESOLVED ? (
                                <Button variant="secondary" onClick={() => updateResolutionStatus(selectedReport, ResolutionStatus.IN_PROGRESS)} className="text-xs rounded-xl">
                                    <Eye className="w-3.5 h-3.5 mr-2" /> Re-open Case
                                </Button>
                            ) : (
                                <Button onClick={() => updateResolutionStatus(selectedReport, ResolutionStatus.CLOSED_RESOLVED)} className="text-xs bg-indigo-600 text-white hover:bg-indigo-700 rounded-xl shadow-lg shadow-indigo-100">
